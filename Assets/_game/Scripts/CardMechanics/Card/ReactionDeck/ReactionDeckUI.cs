...
namespace Littale {
    public class ReactionDeckUI : MonoBehaviour {
        ...
        private CardDataSO pendingReplacementCard; // Card yang menunggu untuk di-replace
        ...
        void HandleReactionCardDrawing(CardDataSO card, int slotIndex) {
        ...
        IEnumerator AnimateReactionCardDraw(GameObject animCard, Transform startPos, Transform endPos, CardDataSO card, int slotIndex) {
        ...
        void HandleReactionCardAdded(CardDataSO card, int slotIndex) {
        ...
        void HandleReactionCardReplaced(CardDataSO oldCard, CardDataSO newCard, int slotIndex) {
        ...
        void HandleReactionTriggered(CardDataSO card, int slotIndex) {
        ...
        void UpdateSlotVisual(int slotIndex) {
            ...
            if (cardImage != null) {
                if (slot.CardDataSO != null) {
                    cardImage.sprite = slot.CardDataSO.icon;
                    cardImage.enabled = true;
                } else {
                    cardImage.enabled = false;
                }
        ...
        void UpdateCooldownVisuals() {
            ...
            for (int i = 0; i < Mathf.Min(slots.Count, reactionSlotVisuals.Count); i++) {
                ...
                if (slot.CardDataSO != null && slot.isOnCooldown) {
                    float cooldownPercentage = slot.cooldownTimer / slot.CardDataSO.cooldown;
        ...
        public void ShowReplaceCardPanel(CardDataSO newCard) {
            ...
            if (replaceCardPanel != null) {
                ...
                for (int i = 0; i < replaceSlotButtons.Length; i++) {
                    if (i < slots.Count && slots[i].CardDataSO != null) {
                        ...
                        if (btnText != null)
                            btnText.text = slots[i].CardDataSO.cardName;
                    } else {
